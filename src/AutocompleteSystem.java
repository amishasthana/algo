...
AutocompleteSystem(String[] sentences, int[] times): This is the constructor. The input is historical data.
 Sentences is a string array consists of previously typed sentences. Times is the corresponding times a sentence has been typed. Your system should
...
public class AutocompleteSystem {
    ...
    class HotSen {
        ...
        public HotSen(String s,int freq) {
            this.s = s;
            f = freq;
        }
    }
    ...
    public AutocompleteSystem(String[] sentences, int[] times) {
        ...
        for(int i = 0;i < sentences.length;i++) {
            //HotSen hs = new HotSen(sentences[i],times[i]);
    ...
    public List<String> input(char c) {
        if (c == '#') {
           ...
           strB.append(c);
            PriorityQueue<HotSen>  setOfSol = getMatched(strB.toString());
            List<String> resList = new LinkedList<>();
            int index = 0;
            while (index < 3) {
                HotSen hs = setOfSol.poll();
                if (hs == null) break;
                resList.add(hs.s);
                System.out.println(hs.s+" "+hs.f);
                index++;
                //if (index == 3) break;
           }
           return resList;
    ...
    private PriorityQueue<HotSen> getMatched(String ss) {
        PriorityQueue<HotSen> topSet = new PriorityQueue<>(3, new Comparator<HotSen>() {
            @Override
            public int compare(HotSen o1, HotSen o2) {
                if (o1.f == o2.f) {
        ...
        for(String s : sMap.keySet()) {
            if (s.startsWith(ss)) {
                HotSen hs = new HotSen(s,sMap.get(s));
                topSet.add(hs);
            }
        }
        return topSet;
